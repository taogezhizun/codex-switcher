...
**当前版本 0.4.4：主窗口更紧凑，修复标题栏与侧栏接缝。** 打开／切换账号移到标题旁，单周期额度卡不再铺满详情区。菜单继续显示本地时区重置时间。 已有账号和设置继续保留，通过 App 内更新即可升级。新安装和完成迁移后的账号操作不访问钥匙串；旧数据仅在你点击“开始迁移”时请求读取授权。文件保存完整登录凭据，未经过应用层加密。详见[本地存储与迁移](docs/USAGE.md#本地存储与迁移)。凭据失效的账号暂停自动刷新并提示重新登录，其他账号照常更新。
